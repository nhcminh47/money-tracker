...
import { generateBudgetRecommendations, generateInsights, getSpendingTrends, getTopSpendingCategories } from '@/lib/ai/insights'
import { useTranslation } from '@/lib/i18n/useTranslation'
import { Lightbulb, Loader, Target, TrendingDown, TrendingUp } from 'lucide-react'
        ...
        <div className='p-6'>
          ...
          <div className='space-y-3'>
            {insights.map((insight, index) => (
              <div
                key={index}
                className='p-3 bg-gradient-to-r from-coral-50 to-cream-100 border border-coral-200 rounded-lg'
              >
              ...
              {trends.map((trend) => (
                ...
                >
                  ...
                  <div className='text-right'>
                    ...
                    <p className='text-xs text-gray-500'>${Math.abs(trend.change).toFixed(2)}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {topCategories.map((category) => (
                ...
                >
                  <div className='flex items-center justify-between'>
                    ...
                    <span className='text-sm font-bold text-gray-900'>${category.totalSpent.toFixed(2)}</span>
                  </div>
                  <div className='flex items-center gap-2'>
                    ...
                    <span className='text-xs text-gray-500 w-12 text-right'>{category.percentOfTotal.toFixed(0)}%</span>
                  </div>
                  <p className='text-xs text-gray-500'>
              ...
              {budgetRecommendations.map((rec) => (
                ...
                >
                  <div className='flex items-center justify-between mb-1'>
                    ...
                    <span className='text-lg font-bold text-green-600'>${rec.suggestedAmount}</span>
                  </div>
