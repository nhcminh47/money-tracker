...
export default function BudgetsClient() {
  ...
  async function loadData() {
    try {
      const [cats, budgets, appSettings] = await Promise.all([
        getAllCategories(),
        getAllBudgetStatuses(),
        getSettings(),
      ]);
  ...
  const categoriesWithoutBudgets = categories.filter(
    (c) => !budgetStatuses.some((s) => s.categoryId === c.id && s.budget)
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 flex items-center justify-between pl-14 md:pl-0">
          ...
          <Button onClick={handleAddBudget} className="hidden md:inline-flex">
        ...
        {categoriesWithoutBudgets.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">
              {t.budgets?.noBudgets || 'Categories Without Budgets'}
            </h2>
