...
import { getSettings, formatCurrency, type AppSettings } from '@/lib/services/settings';
import { seedCategories } from '@/lib/db/utils';
import { db } from '@/lib/db';
import { exportTransactionsCSV, downloadCSV } from '@/lib/utils/export';
...
export default function TransactionsClient() {
    ...
    try {
      ...
      if (filterAccountId) {
        filtered = filtered.filter(
          (tx) => tx.accountId === filterAccountId || tx.toAccountId === filterAccountId
        );
      }
      if (filterStartDate && filterEndDate) {
        filtered = filtered.filter(
          (tx) => tx.date >= filterStartDate && tx.date <= filterEndDate
        );
      }
  ...
  async function handleToggleCleared(id: string) {
    try {
      ...
      showToast(t.transactions?.clearedFailed || 'Failed to update cleared status', 'error');
    }
  }
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    >
                      {transaction.type === 'Income' ? (t.transactions?.income || 'Income') : transaction.type === 'Expense' ? (t.transactions?.expense || 'Expense') : (t.transactions?.transfer || 'Transfer')}
                    </span>
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    {transaction.toAccountId && (
                      <p className="text-sm text-gray-600">→ {getAccountName(transaction.toAccountId)}</p>
                    )}
                  </div>
                  {transaction.categoryId && (
                    <p className="text-sm text-gray-600">{getCategoryName(transaction.categoryId)}</p>
                  )}
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
                  ...
                  >
                    {transaction.type === 'Expense' ? '-' : '+'}{formatCurrency(transaction.amount, settings?.currency)}
                  </p>
                  <div className="flex gap-2 flex-wrap">
                    <Button variant="secondary" size="sm" onClick={() => openEditModal(transaction)}>
                      {t.common?.edit || 'Edit'}
                    </Button>
                    <Button variant="secondary" size="sm" onClick={() => handleDelete(transaction.id)}>
      ...
      <Modal
        ...
        title={editingTransaction ? (t.transactions?.editTransaction || 'Edit Transaction') : (t.transactions?.addTransaction || 'Add Transaction')}
        footer={
          <>
            <Button
              variant="secondary"
      ...
      >
        <form className="space-y-4">{/* Note: form wrapper kept for semantic HTML */}
          <Dropdown
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="cleared-checkbox" className="text-sm text-gray-700 dark:text-gray-300">
